...
export default function JobsPage() {
  ...
  const fetchJobs = async () => {
    try {
      const response = await fetch("https://localhost:7268/api/JobPosting/active");
      if (response.ok) {
        const data = await response.json();
        setJobs(data);
      }
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Açık İş İlanları</h1>
        <p className="text-muted-foreground">
          Restoranlarda çalışma fırsatlarını keşfedin ve başvurun
        </p>
      </div>
